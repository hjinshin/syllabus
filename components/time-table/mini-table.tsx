...
const MiniTimeTable = () => {
  return (
    <table className="h-full w-full table-auto border-collapse select-none">
      <thead>
      ...
      <tbody>
        {Array.from({ length: 10 }, (_, hour) => (
          <tr key={hour}>
            <td>
              <div className="flex h-7 w-8 flex-col items-center justify-center text-neutral-500 rounded-md text-xs text-center transition">
                {hour + 9}
              </div>
            </td>
            {[...Array(5)].map((_, day) => (
